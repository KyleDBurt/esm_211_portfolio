...
```{r}
# install.packages("demography") | in case you don't have it installed
...
```{r}
fr_mort_forecast <- forecast(object = fr_mort_lca,
                             h = 50,
                             se = "innovdrift",
                             jumpchoice = "fit",
                             level = 80)
...
```{r}
# plot the forecasted mortality rate
plot_demog_forecast(
  forecast_data = fr_mort_forecast,
  plot_type = 'mortality') 
```
...
-   'series' defines the demographic series to be analyzed
-   'years' represents the range of years included in the analysis
-   'ages' represents the age groups included in the analysis

#### Q. 2 What do all of the colorful lines represent in the forecasted fertility or mortality rates plot?

-   The lines represent individual years from 2010 to 2050

#### Q. 3 Looking at the forecasted fertility or mortality rates plot, what trends do you observe?

-   Regarding forecasted mortality, there is a large disparity at the early ages but a small disparity at the later ages. This indicates that less people are expected to die at a young age in the future, but the number of older people who are expected to die will largely remain unchanged.
